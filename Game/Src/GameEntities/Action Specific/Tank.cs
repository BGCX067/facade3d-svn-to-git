		...
		void InitTracksTextureAnimation()
		{
			for( int nTrack = 0; nTrack < 2; nTrack++ )
			{
				...
				if( track.meshSubObject != null )
				{
					...
					{
						ShaderBaseMaterial sourceMaterial =
							HighLevelMaterialManager.Instance.GetMaterialByName(
							track.meshSubObject.MaterialName ) as ShaderBaseMaterial;

						if( sourceMaterial != null )
						{
							string materialName = MaterialManager.Instance.GetUniqueName(
								sourceMaterial.Name + "_Cloned" );
