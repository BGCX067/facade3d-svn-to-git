...
namespace GameEntities
{
	...
	public class MeleeWeaponType : WeaponType
	{
		...
		protected override void OnPreloadResources()
		{
			...
			if( !string.IsNullOrEmpty( normalMode.SoundFire ) )
				SoundWorld.Instance.SoundCreate( normalMode.SoundFire, SoundMode.Mode3D );
			if( !string.IsNullOrEmpty( normalMode.SoundKick ) )
				SoundWorld.Instance.SoundCreate( normalMode.SoundKick, SoundMode.Mode3D );
			if( !string.IsNullOrEmpty( alternativeMode.SoundFire ) )
				SoundWorld.Instance.SoundCreate( alternativeMode.SoundFire, SoundMode.Mode3D );
			if( !string.IsNullOrEmpty( alternativeMode.SoundKick ) )
				SoundWorld.Instance.SoundCreate( alternativeMode.SoundKick, SoundMode.Mode3D );
		}
	}
