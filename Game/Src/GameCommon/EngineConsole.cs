...
namespace GameCommon
{
	public sealed class EngineConsole
	{
		...
		public void DoRenderUI( GuiRenderer renderer )
		{
			...
			if( firstRender )
			{
				texture = TextureManager.Instance.Load( "Gui\\Various\\Console.png", Texture.Type.Type2D, 0 );
			...
			Vec2 shadowOffset = new Vec2( 2, 2 ) / viewportSize;

			//Rect textureRect = new Rect( 0, 0, 1, 1 );
			Rect textureRect = new Rect( 0, 0, 10 * renderer.AspectRatio, 10 );
			//textureRect += textureOffset;

			renderer.AddQuad( new Rect( 0, .5f, 1, .5f + shadowOffset.Y ),
				new ColorValue( 0, 0, 0, transparency / 2 ) );
			...
			float staticTextLength = font.GetTextLength( renderer, staticText );
			renderer.AddText( font, staticText, new Vec2( .99f, .5f - font.Height ) + shadowOffset,
				HorizontalAlign.Right, VerticalAlign.Center, new ColorValue( 0, 0, 0, transparency / 2 ) );
			...
			renderer.PushClipRectangle( new Rect( 0, 0, .975f - staticTextLength, 1 ) );
