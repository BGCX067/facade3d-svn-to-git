...
namespace GameCommon
{
	...
	{
		static bool adaptation;
		static float adaptationVelocity = 1;
		static float adaptationMiddleBrightness = .5f;
		static float adaptationMinimum = .5f;
		static float adaptationMaximum = 2;
		static float bloomBrightThreshold = 1;
		static float bloomScale = 1;
		...
		public static bool Adaptation
		{
			get { return adaptation; }
			set { adaptation = value; }
		}

		public static float AdaptationVelocity
		{
			get { return adaptationVelocity; }
			set { adaptationVelocity = value; }
		}

		public static float AdaptationMiddleBrightness
		{
			get { return adaptationMiddleBrightness; }
			set { adaptationMiddleBrightness = value; }
		}

		public static float AdaptationMinimum
		{
			get { return adaptationMinimum; }
			set { adaptationMinimum = value; }
		}

		public static float AdaptationMaximum
		{
			get { return adaptationMaximum; }
			set { adaptationMaximum = value; }
		}

		public static float BloomBrightThreshold
		{
			get { return bloomBrightThreshold; }
			set { bloomBrightThreshold = value; }
		}
				...
				if( hdrBlock != null )
				{
					if( !editor )//No adaptation in the editors
					{
						if( hdrBlock.IsAttributeExist( "adaptation" ) )
							Adaptation = bool.Parse( hdrBlock.GetAttribute( "adaptation" ) );

						if( hdrBlock.IsAttributeExist( "adaptationVelocity" ) )
							AdaptationVelocity = float.Parse( hdrBlock.GetAttribute( "adaptationVelocity" ) );

						if( hdrBlock.IsAttributeExist( "adaptationMiddleBrightness" ) )
							AdaptationMiddleBrightness = float.Parse( hdrBlock.GetAttribute( "adaptationMiddleBrightness" ) );

						if( hdrBlock.IsAttributeExist( "adaptationMinimum" ) )
							AdaptationMinimum = float.Parse( hdrBlock.GetAttribute( "adaptationMinimum" ) );

						if( hdrBlock.IsAttributeExist( "adaptationMaximum" ) )
							AdaptationMaximum = float.Parse( hdrBlock.GetAttribute( "adaptationMaximum" ) );
					}

					if( hdrBlock.IsAttributeExist( "bloomBrightThreshold" ) )
						BloomBrightThreshold = float.Parse( hdrBlock.GetAttribute( "bloomBrightThreshold" ) );

					if( hdrBlock.IsAttributeExist( "bloomScale" ) )
						BloomScale = float.Parse( hdrBlock.GetAttribute( "bloomScale" ) );
				}
			}
		}
