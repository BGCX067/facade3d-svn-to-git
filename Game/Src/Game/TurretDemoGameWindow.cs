	...
	public class TurretDemoGameWindow : ActionGameWindow
	{
		...
		void Map_Tick( Entity entity )
		{
			...
			//Update placard text
			{
				...
				GameGuiObject placard = (GameGuiObject)Entities.Instance.GetByName( "Placard_Game" );
				placard.MainControl.Controls[ "MainText" ].Text = mainText;
				placard.MainControl.Controls[ "DownText" ].Text = downText;
			}
		...
		void GameStart()
		{
			...
			if( turret != null )
				turret.Life = turret.Type.LifeMax;
		...
		void UpdateVictoryObjects( bool visible )
		{
			foreach( Entity entity in Map.Instance.Children )
			{
				if( entity.TextUserData != "Victory" )
					continue;
		...
		void EnemyUnitTick( Entity entity )
		{
			...
			if( playerUnit != null )
			{
				...
				if( allowSuicide && playerDistance < suicideDistance )
				{
					...
					Dynamic dieObject = (Dynamic)Entities.Instance.Create(
						"MonsterSuicideObject", Map.Instance );
